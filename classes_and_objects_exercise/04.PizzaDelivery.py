class PizzaDelivery():
    ...
    def add_extra(self, ingredients: str, quantity: int, price_per_quantity: float):
        if not self.ordered:
            self.ingredients[ingredients] = self.ingredients.get(ingredients, 0) + quantity
            self.price += price_per_quantity * quantity
        
        return f"Pizza {self.name} already prepared, and we can't make any changes!"
    
    def remove_ingredients(self, ingredient: str, quantity: int, price_per_quality: float):
        if not self.ordered:
            ...
            self.price -= price_per_quality * quantity
        
        return f"Pizza {self.name} already prepared, and we can't make any changes!"
    
    def make_order(self):
        if not self.make_ordered:
            self.ordered = True
